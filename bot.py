...
def send_text_message(contact_number,msg):
   payload = json.dumps({
    ...
    # post request
   response = requests.request("POST", facebook_url, headers=facebokk_headers, data=payload)
   print('response',response) # print('response',response.json())
...
def webhhook():
    if request.method == 'POST':
        ...
        try:
            ...
            if json_data['object'] == "whatsapp_business_account" and json_data.get('entry')[0].get('changes')[0].get('value').get('contacts') is not None:
        ...
        if msg_type == "text":
            msg_text = json_data['entry'][0]['changes'][0]['value']['messages'][0]['text']['body']
